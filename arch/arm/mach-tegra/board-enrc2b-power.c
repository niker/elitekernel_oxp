/*
 * arch/arm/mach-tegra/board-enrc2b-power.c
 *
...
static struct regulator_consumer_supply tps80031_smps4_supply_common[] = {
	REGULATOR_SUPPLY("v_mmc_sr_2v85", NULL),
};
...
static struct regulator_consumer_supply tps80031_ldo3_supply_common[] = {
	REGULATOR_SUPPLY("v_vib_1v6", NULL),
};
...
TPS_PDATA_INIT(ldo5, common, 1000, 3300, 0, 0, 0, 0, 2850, 0, 1, 0, 0, 0);
TPS_PDATA_INIT(ldo6, common, 1000, 3300, 0, 0, 0, 0, 2850, 0, 1, 0, 0, 0);
...
static struct tps80032_charger_platform_data pdata_charger = {
	.linch_gated_irq = ENT_TPS80031_IRQ_BASE + TPS80031_INT_LINCH_GATED,
	.fault_wdg_irq = ENT_TPS80031_IRQ_BASE + TPS80031_INT_FAULT_WDG,
	.int_chg_irq = ENT_TPS80031_IRQ_BASE + TPS80031_INT_INT_CHRG,
	.use_hiz_disable = 0,
	.sw_temp_25_gpio = TEGRA_GPIO_INVALID,
};

static struct tps80032_gauge_platform_data pdata_gauge = {
	.irq = ENT_TPS80031_IRQ_BASE + TPS80031_INT_CC_AUTOCAL,
	.adc_volt_channel = 0,
};
...
static struct regulator_consumer_supply fixed_reg_lcm_3v_en_supply[] = {
        REGULATOR_SUPPLY("v_lcm_3v", NULL),
};

static struct regulator_consumer_supply fixed_reg_aud_2v85_en_supply[] = {
        REGULATOR_SUPPLY("v_aud_2v85", NULL),
};
...
FIXED_REG(4, lcmio_1v8_en,   "tps80031_VIO", TEGRA_GPIO_PE5,  true, 1800, 1, 0);
FIXED_REG(5, srio_1v8_en,    "tps80031_VIO", TEGRA_GPIO_PY3,  true, 1800, 1, 0);
FIXED_REG(6, cam2_d1v2_en,   "tps80031_VIO", TEGRA_GPIO_PF6,  true, 1200, 0, 0);
FIXED_REG(7, cam_d1v2_en,    "tps80031_VIO", TEGRA_GPIO_PF5,  true, 1200, 0, 0);
FIXED_REG(8, camio_1v8_en,   "tps80031_LDO7",      TEGRA_GPIO_PBB4, true, 1800, 0, 0);
FIXED_REG(9, mhl_1v2_en,     NULL,      TEGRA_GPIO_PE4,  true, 1200, 0, 0);
...
static int __init enrc2b_fixed_regulator_init(void)
{
	int i;
...
static int __init enrc2b_regulators_fixed_gpio_init(void)
{
	int ret;

	if (!machine_is_enrc2b())
		return 0;

	ret = enrc2b_fixed_regulator_init();
	if (ret)
		return ret;

	ret = enrc2b_gpio_regulator_init();
	return ret;
}
subsys_initcall_sync(enrc2b_regulators_fixed_gpio_init);

static struct i2c_board_info __initdata enrc2b_regulators[] = {
	{
...
#ifdef CONFIG_PM
static void enrc2b_suspend_work(void)
{
...
static void enrc2b_resume_work(void)
{
...
static int __init enrc2b_gpio_rtc_init(void)
{
	int ret;
...
#ifdef CONFIG_PM
	tps_platform.suspend_work = enrc2b_suspend_work;
	tps_platform.resume_work = enrc2b_resume_work;
#endif
	return 0;
...
int __init enrc2b_regulator_init(void)
{
	...
	enrc2b_gpio_rtc_init();

	if (board_mfg_mode() == BOARD_MFG_MODE_FACTORY2)
		pdata_charger.use_hiz_disable = 1;

	// TODO: test HW phase before assign subdevs
	tps_platform.num_subdevs = ARRAY_SIZE(tps80031_devs);
	tps_platform.subdevs = tps80031_devs;

	i2c_register_board_info(4, enrc2b_regulators,
		    ARRAY_SIZE(enrc2b_regulators));

	enrc2b_tsensor_init();
	return 0;
}

static void enrc2b_board_suspend(int lp_state, enum suspend_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_SUSPEND_BEFORE_CPU))
		tegra_console_uart_suspend();
}

static void enrc2b_board_resume(int lp_state, enum resume_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_RESUME_AFTER_CPU))
		tegra_console_uart_resume();
}

static struct tegra_suspend_platform_data enrc2b_suspend_data = {
	.cpu_timer	= 600,
	.cpu_off_timer	= 200,
...
{
	tegra_init_suspend(&enrc2b_suspend_data);
	return 0;
}

#ifdef CONFIG_TEGRA_EDP_LIMITS
int __init enrc2b_edp_init(void)
{
	unsigned int regulator_mA;

	regulator_mA = get_maximum_cpu_current_supported();
	//if (!regulator_mA) {
		regulator_mA = 8200; /* regular AP37 */
	//}
...
static struct platform_device enrc2b_bpc_mgmt_device = {
	.name		= "tegra-bpc-mgmt",
	.id		= -1,
	.dev		= {
...
void __init enrc2b_bpc_mgmt_init(void)
{
...
#endif
	platform_device_register(&enrc2b_bpc_mgmt_device);
